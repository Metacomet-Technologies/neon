    ...
    public function handle(): void
    {
        ...
        if (empty($this->imageAttachments)) {
            ...
            $this->updateNativeCommandRequestFailed(
                status: 'failed',
                message: 'No image attachments provided.',
                statusCode: 400,
            );
            return;
        }
        ...
        try {
            ...
            if (empty($analysisResults)) {
                $this->sendErrorMessage('Failed to analyze any images. Please try again with valid Discord server screenshots.');
                return;
            }
            ...
            if (!$finalAnalysis) {
                $this->sendErrorMessage('Failed to generate server template from image analysis.');
                return;
            }
            ...
            Log::error('ProcessImageAnalysisJob failed', [
                'error' => $e->getMessage(),
                'image_count' => count($this->imageAttachments),
                'discord_user_id' => $this->discordUserId,
                'channel_id' => $this->channelId,
            ]);

            $this->sendErrorMessage('An error occurred while analyzing the image(s). Please try again.');
            $this->updateNativeCommandRequestFailed(
                status: 'error',
...
Focus on recreating the essence and organization of the analyzed server while using valid Discord bot commands and enhancing the creative appeal! 🚀";
    ...
    private function getAvailableDiscordCommands(): string
    {
        ...
        return Cache::remember('neon_available_commands', now()->addHours(1), function () {
            try {
                $commands = \Illuminate\Support\Facades\DB::table('native_commands')
    ...
    {
        $availableCommands = \Illuminate\Support\Facades\DB::table('native_commands')
            ->where('is_active', true)
            ->whereNotIn('slug', ['neon', 'analyze-server', 'template-from-image'])
            ->pluck('slug')
            ->toArray();
