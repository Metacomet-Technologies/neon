...
final class ProcessListRolesJob extends ProcessBaseJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (empty($filteredRoles)) {
                ...
                $this->updateNativeCommandRequestSuccess(
                    status: 'completed',
                    message: 'Successfully listed server roles (none found).',
                );
                return;
            }
            ...
            $this->updateNativeCommandRequestSuccess(
                status: 'completed',
                message: 'Successfully listed server roles.',
            );
    ...
    /**
     * Update the native command request as successful
     */
    private function updateNativeCommandRequestSuccess(string $status, string $message): void
    {
